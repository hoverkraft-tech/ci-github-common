...
export class SummaryFormatter {
  ...
  format(reportData, reportName, includePassed = false) {
    ...
    if (reportData.getTotalTests() > 0) {
      sections.push(this._formatTestResults(reportData, includePassed));
    }
  ...
  _formatTestResults(reportData, includePassed) {
  ...
  _formatLintIssues(reportData) {
    ...
    if (errors.length > 0) {
      ...
      for (const issue of errors.slice(0, 10)) { // Limit to first 10
      ...
      if (errors.length > 10) {
        output += `_... and ${errors.length - 10} more errors_\n\n`;
      }
    }
    ...
    if (warnings.length > 0) {
      ...
      for (const issue of warnings.slice(0, 5)) { // Limit to first 5
        output += `- **\`${issue.file}:${issue.line}:${issue.column}\`**\n`;
        output += `  - Rule: \`${issue.rule}\`\n`;
        output += `  - ${this._escapeMarkdown(issue.message)}\n\n`;
      }
      ...
      if (warnings.length > 5) {
        output += `_... and ${warnings.length - 5} more warnings_\n\n`;
      }
    }
